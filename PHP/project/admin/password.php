                        ...
                        <div class="col-lg-12 mb-4">
                            <div class="card shadow mb-4">
                                ...
                                <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                    ...
                                    <div class="my-3">
                                        <label for="cnpass">Enter Confirm Password</label>
                                        <input type="password" name="cnpass" id="cnpass" required class="form-control">
                                    </div>
                                    <div class="my-3">
                                        <input type="submit" value="Update Password" class="btn btn-primary" name="updateProcess">
    ...
    if(isset($_POST["updateProcess"])){
        ...
        if($npass === $cnpass){
            ...
            if($users->adminLogin($adminemail, $cpass)){
                ...
                $users->logWriter($adminemail, "Password Changed Successfully");
            }else{
                $_SESSION["msg"] = "<div class='alert alert-danger alert-dismissible'>
                <button type='button' class='close' data-dismiss='alert'>&times;</button>
                <strong>Error! </strong> Invalid Current Password.</div>";    
                $users->logWriter($adminemail, "Invalid Attempt of Password Change");
            }
        }else{
            $_SESSION["msg"] = "<div class='alert alert-danger alert-dismissible'>
            <button type='button' class='close' data-dismiss='alert'>&times;</button>
            <strong>Error! </strong> Confirm Pasword does not Match.</div>";
        }
